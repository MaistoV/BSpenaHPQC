...
Apache Hadoop software library is a `framework` used for the distributed processing of massive datasets across `clusters` of commodity hardware. `Hadoop Cluster` is a computational cluster used for storing and analyzing huge amounts of unstructured or structured data in a distributed computing environment. 

The Hadoop architecture has main three layers<sup>[[1]](References.md#hadoop)</sup>:
...
Master and slaves nodes contain the components related to the main three layers.
...
## HDFS <a name="hdfs"></a>
HDFS (Hadoop Distributed File System)<sup>[[3]](References.md#hdfs_architecture)</sup> is a java-based distributed file system running on commodity hardware which supports a traditional hierarchical file organization. This file system :
...
MapReduce<sup>[[2]](References.md#hadoop_architecture)</sup><sup>[[4]](References.md#mapred_tutorial)</sup> is a software framework for writing applications (which need not be written in Java) that process large datasets in parallel on large clusters. The framework works on sets of `<key,value> pairs`.
...
* It is an execution of `map tasks` and `reduce tasks`. Each map/reduce task uses a user-defined `map/reduce function` which is implemented through interfaces and/or abstract-classes. MapReduce creates a map task for each split, instead the number of reduce task can be set by the user (set as zero if no reduction is desired).

MapReduce has the following phases<sup>[[4]](References.md#mapred_tutorial)</sup><sup>[[5]](References.md#mapred_flow)</sup> : 
...
YARN (Yet Another Resource Negotiator)<sup>[[6]](References.md#yarn)</sup><sup>[[7]](References.md#yarn_intro)</sup><sup>[[8]](References.md#yarn_tutorial)</sup> is a framework for distributed computing which separates resource management and processing components.  
YARN :
* Allows the execution of an `application` (`single MapReduce job` or `DAG of jobs`). 
...
* `ResourceManager` : The `master daemon` runs on master node. It manages the resources among all the applications in the system. The daemon assigns map and reduce tasks to the NodeManager and schedules containers. The ResourceManager has two main components :
    ...
    * `ApplicationsManager` : Accepts `job submissions` by the clients<sup>[[9]](References.md#yarn_app)</sup> and secures resources on a node (an operation known as "`negotiating the first container`") to launch the ApplicationMaster.
...
ResourceManager/NodeManager rapresent the `data-computation framework`.

YARN supports the following schedulers<sup>[[7]](References.md#yarn_intro)</sup> :
...
* `Fair scheduler` : Organizes applications into queues or pools and allows to share resources fairly between queues (every application belongs to a queue). It grants container with the least amount of allocated resources.
...
### Supported Modes
Hadoop supports three different modes<sup>[[10]](References.md#modes)</sup> :
...
TestDFSIO benchmark is a `read and write test` for HDFS<sup>[[11]](References.md#dfsio)</sup> and describes how faster a cluster is in terms of I/O. Benchmark results are saved in *TestDFSIO_results.log* file (in the current local directory) and printed to stdout.
